...
template<int _Dim>
Eigen::Matrix<double, _Dim, _Dim> df_dx(state_ikfom s, input_ikfom in)
{
	...
	if (_Dim == 27)
	{
		cov.template block<3, 3>(0, 24) = Matrix3d::Identity(); 	//位置--虚拟零偏
		cov.template block<3, 3>(24, 24) = X;					   //虚拟零偏--虚拟零偏
		cov.template block<3, 3>(24, 15) = Z;					   //虚拟零偏--陀螺仪零偏
	}
	return cov;
}
...
Eigen::Matrix<double, _Dim, 12> df_dw(state_ikfom s, input_ikfom in)
{
	...
	if (_Dim == 27)
	{
		cov.template block<3, 3>(24, 6) = Sophus::SO3::hat(s.pos) * s.rot.matrix();    //虚拟零偏--陀螺仪零偏噪声
	}
	return cov;
}
